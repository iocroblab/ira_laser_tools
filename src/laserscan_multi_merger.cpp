...
void LaserscanMerger::laserscan_topic_parser()
{
	...
	if( (tmp_input_topics.size() != input_topics.size()) || !equal(tmp_input_topics.begin(), tmp_input_topics.end(), input_topics.begin()))
	{
		...
		if(input_topics.size() > 0)
		{
			...
			{
                scan_subscribers[i] = node_.subscribe<sensor_msgs::LaserScan> (input_topics[i].c_str(), 1, boost::bind(&LaserscanMerger::scanCallback,this, boost::placeholders::_1, input_topics[i]));
...
int main(int argc, char** argv)
{
    ...
    f = boost::bind(&LaserscanMerger::reconfigureCallback, &_laser_merger, boost::placeholders::_1,boost::placeholders::_2);
